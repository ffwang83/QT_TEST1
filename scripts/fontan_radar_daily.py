...
def normalize(r,day):
    brief=r.get('brief') or {}; detail=r.get('detail') or {}
    title=brief.get('title') or r.get('title') or ''; agency=r.get('unit_name') or r.get('agency') or ''; no=r.get('job_number') or r.get('procurement_no') or ''
    raw=' '.join(str(v) for v in detail.values()); text=' '.join([title,agency,raw,brief.get('type','')])
    budget=parse_money(detail.get('預算金額') or detail.get('預算') or detail.get('採購金額') or r.get('budget'))
    fit=text_score(text,budget); neg=[k for k in NEG if k in text]; pos=[k for k in POS if k in text]
    decision='排除' if len(neg)>=2 or fit<50 else ('保留' if fit>=80 else '研究')
